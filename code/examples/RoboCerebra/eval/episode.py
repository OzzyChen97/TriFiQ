...
from pi05_policy import execute_policy_step  # noqa: F401  (re-exported for eval_pi05)
...
def execute_policy_step(
    ...
    """Run pi0.5 policy inference and pop one action from the queue.

    ``model`` here is the pi0.5 websocket client (see pi05_policy.py).
    The extra OpenVLA args are kept for call-site compatibility and ignored.
    """
    from pi05_policy import execute_policy_step as _pi05_step

    return _pi05_step(cfg, model, observation, desc, action_queue)
