#!/usr/bin/env python3
"""Utility functions for RoboCerebra evaluation."""
...
def load_actions(json_path: str) -> Dict[str, List[List[str]]]:
    """Load actions from goal.json in old or step-annotated format."""
    ...
    for obj_id, relations in data.items():
        ...
        for item in relations:
            ...
            if (
                ...
                and "state_pair" in item
                and "task_step" in item
            ):
...
def determine_scene_type(bddl_file: Path) -> str:
    ...
    logger.warning(
        "Could not determine scene type for "
        f"{filename}, defaulting to COFFEE_TABLESCENE"
    )
...
def load_init_state(
    ...
    - Ideal, Observation_Mismatching, and Random_Disturbance all use
      init files from the Ideal directory
    """
    ...
    if task_type in use_ideal_files:
        ...
        log_message(
            f"Using Ideal init files for task type: {task_type}",
            log_file,
        )
    else:
    ...
    if init_file.exists():
        try:
            with open(init_file, "rb") as file_obj:
                init_state = pickle.load(file_obj)
            # Avoid circular import by importing locally
            from robocerebra_logging import log_message
...
def get_task_directories(cfg: GenerateConfig) -> List[Tuple[str, Path]]:
    ...
    - Ideal, Observation_Mismatching, and Random_Disturbance all use
      files from the Ideal directory
    - Other task types use their original directories
    """
    robocerebra_root = Path(cfg.robocerebra_root)
    task_dirs = []
    cases_filter = (
        {c.strip() for c in cfg.cases.split(",") if c.strip()}
        if getattr(cfg, "cases", "")
        else set()
    )
    ...
    for task_type in cfg.task_types:
        if task_type in use_ideal_files:
            ...
            logger.info(
                f"Using Ideal directory files for task type: {task_type}"
            )
        else:
            # Use files from the original task type directory
            source_dir = robocerebra_root / task_type
            logger.info(
                "Using original directory files for task type: "
                f"{task_type}"
            )
        ...
        for case_dir in source_dir.iterdir():
            if case_dir.is_dir():
                if cases_filter and case_dir.name not in cases_filter:
                    continue
...
def process_action(action, model_family):
    # pi0.5 outputs 7-dim actions already in the LIBERO env action space
    # (delta pose + gripper in [-1, 1]); no conversion is needed, unlike OpenVLA.
    return action


# ★★ Find the address (addr) of an object's y-axis in qpos ★★
def _find_obj_y_addr(sim, obj_name: str) -> Optional[int]:
    """Return the y-coordinate index of an object's qpos entry."""
    # Three common naming conventions:
    # <name>_1_joint0 / <name>_joint0 / <name>_joint
    patterns = [
        f"{obj_name}_1_joint0",
        f"{obj_name}_joint0",
        f"{obj_name}_joint",
    ]
    # logger.debug(
    #     f"[DEBUG] Searching for object '{obj_name}' joints: {patterns}"
    # )
    ...
    for jn in patterns:
        if jn in sim.model.joint_names:
            ...
            # logger.debug(
            #     f"[DEBUG] Found joint '{jn}' for object '{obj_name}' "
            #     f"at qpos address {qpos_addr}, y-addr = {qpos_addr + 1}"
            # )
            return qpos_addr + 1  # x,y,z -> take y

    # logger.debug(
    #     f"[DEBUG] No joint found for object '{obj_name}', "
    #     f"available joints: {list(sim.model.joint_names)[:10]}..."
    # )
    return None
...
def _load_step_objects(json_path: str, step_desc: Sequence[str]) -> List[str]:
    ...
    Args:
        ...
        step_desc: List obtained from parse_task_description()
    Returns:
        A list of object names corresponding to step_desc.
    """
    with open(json_path, "r", encoding="utf-8") as file_obj:
        data = json.load(file_obj)
    # Create a mapping from 'Step: xxx' to object
    mapping = {
        item["step"]: item["object"] for item in data if "step" in item
    }
    objs = []
...
def parse_task_description(txt_path: str) -> Tuple[List[str], List[int]]:
    ...
    logger.debug(
        f"Parsing task description file: {txt_path} with {len(lines)} lines"
    )
        ...
        if line.startswith("Step"):
            ...
            if i + 1 < len(lines):
                ...
                if m:
                    ...
                    logger.warning(
                        "Expected bracket line after step, but found: "
                        f"{lines[i + 1]}"
                    )
            else:
    ...
    logger.info(
        f"Parsed {len(step_desc)} steps and {len(start_indices)} indices "
        f"from {txt_path}"
    )
    return step_desc, start_indices
...
def setup_dynamic_distractor_info(
    ...
    # log_message(
    #     f"[DEBUG] Dynamic setup: cfg.dynamic={cfg.dynamic}, "
    #     f"cfg.resume={cfg.resume}",
    #     log_file,
    # )

    if not (cfg.dynamic and cfg.resume):
        # log_message(
        #     f"[DEBUG] Dynamic disabled: dynamic={cfg.dynamic}, "
        #     f"resume={cfg.resume}",
        #     log_file,
        # )
        return None
    ...
    # log_message(
    #     f"[DEBUG] Checking dynamic setup for task: {task_dir.name}",
    #     log_file,
    # )
    ...
    # log_message(
    #     f"[DEBUG] Processing {len(step_objects)} step objects for "
    #     "related addresses",
    #     log_file,
    # )
    ...
    for i, obj_name in enumerate(step_objects):
        # log_message(
        #     f"[DEBUG] Step {i}: object '{obj_name}' in MOVABLE_LIST: "
        #     f"{obj_name in MOVABLE_OBJECT_LIST if obj_name else False}",
        #     log_file,
        # )
        ...
        if addr is None:
            ...
            # log_message(
            #     f"[DEBUG] Found address {addr} for object {obj_name}",
            #     log_file,
            # )
    ...
    # log_message(
    #     "[DEBUG] Related objects found: "
    #     f"{sum(1 for x in step_addr_y if x is not None)}/"
    #     f"{len(step_addr_y)}",
    #     log_file,
    # )
    ...
    unrelated_addr = []
    # log_message(
    #     "[DEBUG] Searching for unrelated objects in "
    #     f"{len(MOVABLE_OBJECT_LIST)} candidates",
    #     log_file,
    # )
    ...
    for name in MOVABLE_OBJECT_LIST:
        ...
        if addr is not None:
            # log_message(
            #     f"[DEBUG] Found unrelated object {name} at address {addr}",
            #     log_file,
            # )
            unrelated_addr.append((addr, env.sim.data.qpos[addr].copy()))

    # log_message(
    #     f"[DEBUG] Unrelated objects found: {len(unrelated_addr)}",
    #     log_file,
    # )
    ...
    # log_message(
    #     f"[DEBUG] Dynamic validation: has_related={has_related}, "
    #     f"has_unrelated={has_unrelated}",
    #     log_file,
    # )
    ...
    if has_related and has_unrelated:
        ...
        log_message(
            ...
            f"(related: {has_related}, unrelated: {has_unrelated}), "
            "feature disabled.",
            log_file,
        )
        return None
