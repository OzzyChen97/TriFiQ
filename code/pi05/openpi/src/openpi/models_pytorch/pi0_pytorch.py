...
def get_safe_dtype(target_dtype, device_type):
    ...
    if device_type == "cpu":
        # Keep CPU execution on the broadly supported reference path.
...
class PI0Pytorch(nn.Module):
    ...
    def forward(self, observation, actions, noise=None, time=None) -> Tensor:
        ...
        if suffix_embs.dtype != transformer_dtype:
            suffix_embs = suffix_embs.to(dtype=transformer_dtype)
        if prefix_embs.dtype != transformer_dtype:
        ...
        def action_out_proj_func(suffix_out):
            result = self.action_out_proj(suffix_out.to(dtype=self.action_out_proj.weight.dtype))
    ...
    def sample_actions(
        ...
        if trajectory is not None:
            ...
            # inference retains its tensor return type.
