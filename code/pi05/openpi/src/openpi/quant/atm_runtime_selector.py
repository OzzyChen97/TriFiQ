"""Frozen calibration-gated correction selector for OpenPI pi0.5 serving.
...
RUNTIME_SELECTOR_PATH_ENV = "OPENPI_RUNTIME_SELECTOR_PATH"
RUNTIME_SELECTOR_MODEL_ENV = "OPENPI_RUNTIME_SELECTOR_MODEL"
RUNTIME_SELECTOR_STRICT_ENV = "OPENPI_RUNTIME_SELECTOR_STRICT"
...
class RuntimeSelector:
    ...
    def from_env(cls) -> "RuntimeSelector | None":
        ...
        model_id = os.environ.get(RUNTIME_SELECTOR_MODEL_ENV, "pi05")
        strict = os.environ.get(RUNTIME_SELECTOR_STRICT_ENV, "1") not in ("0", "false", "False", "")
    ...
    def metadata(self) -> dict[str, Any]:
        ...
        encoded_mapping = json.dumps(mapping, sort_keys=True, separators=(",", ":")).encode("utf-8")
        return {
            "enabled": True,
...
_CURRENT_DECISION: contextvars.ContextVar[RuntimeSelectorDecision | None] = contextvars.ContextVar(
    "openpi_runtime_selector_decision", default=None
)
...
def atm_enabled_for_current_request() -> bool:
    ...
    # In true runtime-selector mode, missing context must never accidentally
    # apply all loaded scaling values. The websocket layer normally fails before
    # inference if metadata is missing; this is the fail-safe path.
