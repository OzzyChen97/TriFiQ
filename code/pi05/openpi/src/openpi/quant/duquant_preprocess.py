...
def compute_block_rotation(W_block: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        # Only U is used.  Economy SVD avoids materializing an enormous Vh for
        # wide matrices such as 64x16384 while producing the same 64x64 U.
        U_torch, _, _ = torch.linalg.svd(X_torch, full_matrices=False)
        U = U_torch.cpu().numpy().astype(np.float64)
    except Exception:
        # Fallback to CPU NumPy SVD
        try:
            U, _, _ = np.linalg.svd(X.astype(np.float64, copy=False), full_matrices=False)
...
def pack_weight(
    ...
    *,
    ...
    if os.environ.get("OPENPI_DUQUANT_HESSIAN_W4_PATH"):
    ...
    for b in range(n_row_blocks):
        ...
        try:
            ...
            U_torch, _, _ = torch.linalg.svd(W_rows_torch, full_matrices=False)
            U = U_torch.cpu().numpy().astype(np.float64)
        except Exception:
            # Fallback to CPU NumPy SVD
            try:
                U, _, _ = np.linalg.svd(W_rows.astype(np.float64, copy=False), full_matrices=False)
            except np.linalg.LinAlgError:
...
class PercentileCalibrator:
    ...
    def mark_full(self) -> None:
        """Mark a calibrator populated by a validated static artifact as full."""
        self._seen = self.max_batches
...
def apply_input_transform_optimized(
    ...
    if R_in_cache:
        ...
        for b in range(n_blocks):
            ...
            R = R_in_cache[b][: (end - start), : (end - start)].to(
                device=x_view.device, dtype=x_view.dtype
            )
...
def apply_output_restore_optimized(
    ...
    for b in range(n_row_blocks):
        ...
        rs = b * block_out_size
        re = min((b + 1) * block_out_size, out_features)
        Rb = R_out_cache[b][: (re - rs), : (re - rs)].to(
            device=y_view.device, dtype=y_view.dtype
        )
...
def transform_weight_for_forward_optimized(
    ...
    *,
    ...
    if perm_cache is not None:
        W_t = W_t.index_select(dim=1, index=perm_cache.to(device=W_t.device))
    ...
    if R_in_cache:
        ...
        for b in range(n_blocks):
            ...
            R = R_in_cache[b][: (end - start), : (end - start)].to(
                device=W_t.device, dtype=W_t.dtype
            )
