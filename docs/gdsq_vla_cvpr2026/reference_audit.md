...
- Table 1 enables the complete 50-task GR00T $\Omega$-QVLA row and its independently audited storage cells; the $\pi_{0.5}$ row remains claim-disabled. Table 2 (`tables/omega_qvla_libero.tex`) contains only local FP16, QuantVLA W4A8, Uniform W6, $\Omega$-QVLA W4A4, and GDSQ-VLA rows; no author-reported values are shown, and all cells remain disabled until exact 4,000-episode joint coverage and SHA checks pass.
