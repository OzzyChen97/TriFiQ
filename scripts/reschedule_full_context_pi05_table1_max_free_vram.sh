...
PARENT_SCHEDULE="$RUN_ROOT/manifest.schedule_all_free_v1.json"
SCHEDULE="$RUN_ROOT/manifest.schedule_max_free_vram_v2.json"
...
# Order is the immutable task-shard order. Every split has at least 16 tasks,
# so 16 is the largest useful shard count without an empty evaluator shard.
SERVER_ROWS=(
    ...
    "g1_s1,1,19811,dypac_pi05_table1_g1_s1"
    "g4_s1,4,19841,dypac_pi05_table1_g4_s1"
    "g5_s1,5,19851,dypac_pi05_table1_g5_s1"
    "g6_s1,6,19861,dypac_pi05_table1_g6_s1"
    "g7_s1,7,19871,dypac_pi05_table1_g7_s1"
...
start_one() {
    local gpu="$1" port="$2" instance="$3" pid=""
    ...
    server_env "$SERVER_MANAGER" start "$CONFIG_ID" "$gpu" "$port" "$instance" \
        >"$CONTROL_DIR/$instance.maxvram_start.log" 2>&1
}

start_extra_wave() {
    local wave="$1" starts=() labels=() row label gpu port instance failed=0
    for row in "${SERVER_ROWS[@]}"; do
        IFS=, read -r label gpu port instance <<<"$row"
        [[ "$label" == *_"$wave" ]] || continue
        start_one "$gpu" "$port" "$instance" &
    ...
    for index in "${!starts[@]}"; do
        if ! wait "${starts[$index]}"; then
            ...
            tail -n 100 "$CONTROL_DIR/${labels[$index]}.maxvram_start.log" >&2 || true
        fi
    done
    [[ "$failed" == 0 ]]
}
...
smoke_extra() {
    local instance="dypac_pi05_table1_g1_s1" port=19811 gpu=1 runtime hash output
    runtime="$CONTROL_DIR/$instance.runtime.json"; hash="$(metadata_hash "$runtime")"
    output="$PREFLIGHT_DIR/maxvram_smoke_${hash:0:12}.jsonl"
    PYTHONPATH="$CLIENT_PATH${PYTHONPATH:+:$PYTHONPATH}" \
        ...
        >"$PREFLIGHT_DIR/maxvram_smoke.log" 2>&1
    echo "extra-server smoke passed: $output"
}
...
build_schedule_args() {
    SCHEDULE_ARGS=(--base-manifest "$BASE_MANIFEST" --parent-schedule "$PARENT_SCHEDULE" \
        --run-dir "$RUN_ROOT" --scheduler "$REPO_ROOT/scripts/reschedule_full_context_pi05_table1_max_free_vram.sh" \
        --out "$SCHEDULE")
    local index row label gpu port instance runtime suffix seeds worker_id
    for index in "${!SERVER_ROWS[@]}"; do
        row="${SERVER_ROWS[$index]}"; IFS=, read -r label gpu port instance <<<"$row"
        ...
        for suffix in lo hi; do
            ...
            worker_id="maxvram_${label}_${suffix}"
