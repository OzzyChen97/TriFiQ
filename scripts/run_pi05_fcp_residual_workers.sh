...
MATERIALIZER="$REPO_ROOT/scripts/tools/materialize_pi05_fcp_residual_schedule.py"
AGGREGATOR="$REPO_ROOT/scripts/run_pi05_fcp_diagnostic.sh"
SCHEDULE="$ROOT/residual_schedule.json"
...
runner_alive() {
    ...
    runner_id="$1"
    pid_file="$WORKER_CONTROL/$runner_id.pid"
    [[ -f "$pid_file" ]] || return 1
    pid="$(<"$pid_file")"; [[ "$pid" =~ ^[0-9]+$ ]] || return 1
    kill -0 "$pid" 2>/dev/null || return 1
    command="$(ps -p "$pid" -o args=)"
    [[ "$command" == *run_pi05_fcp_residual_workers.sh*runner* ]]
}
...
run_queue() {
    ...
    echo "residual runner complete: $runner_id"
}
...
launch() {
    ...
    nohup setsid "$0" supervise >"$CONTROL/residual_supervisor.log" 2>&1 </dev/null &
    echo $! >"$CONTROL/residual_supervisor.pid"
    "$AGGREGATOR" status
}

supervise() {
    local count index runner_id completed
    count="$(jq '.runners | length' "$SCHEDULE")"
    while true; do
        alive=0
        for index in $(seq 0 $((count - 1))); do
    ...
    if rg -n "Traceback|Segmentation fault|CUDA out of memory|CUDNN_STATUS|CUBLAS_STATUS|unable to find an engine" "$WORKER_CONTROL"/residual_*.log; then
        echo "residual rollout contains a failure" >&2; return 1
    fi
    completed="$({ rg -l 'residual runner complete:' "$WORKER_CONTROL"/residual_*.log || true; } | wc -l)"
    [[ "$completed" -eq "$count" ]] || { echo "residual completion count $completed != $count" >&2; return 1; }
    "$AGGREGATOR" status
