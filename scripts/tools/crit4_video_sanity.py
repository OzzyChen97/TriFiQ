...
env = RoboCasaGymEnv(env_name=TASK, enable_render=True, split="target")
...
for i in range(N):
    ...
    chunks = _normalize_action_chunks(chunk)
    action = {f"action.{k}": np.atleast_1d(v[0]) for k, v in chunks.items()}
    acts.append({
