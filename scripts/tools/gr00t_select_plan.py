...
def build_scores(
    ...
    cka_field ∈ {"cka", "cka_dit"} selects the measurement location of the CKA
    term (v1.4 D-024: cka_dit = DiT-output CKA, the only location whose
    1−CKA correlates with d_solver/D_func).
    """
...
def lambda_sweep_plans(
    ...
    out: List[Dict[str, Any]] = []
    for lc, ls in pairs:
        sc = build_scores(sensitivity, layer_names, lc, ls, cka_field)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cka-field", default="cka", choices=["cka", "cka_dit"],
                   help="v1.4 (D-024): CKA measurement location for the λ_cka term. 'cka' = raw "
                        "Linear (v1.3, no signal); 'cka_dit' = DiT output (rho 0.72-0.95 vs "
                        "d_solver, 0.55-0.94 vs D_func — use for the criterion-4 CS+CKA plans).")
