#!/usr/bin/env python3
"""Validate and merge the four LIBERO DyPAC FP16 on-policy shards."""

from __future__ import annotations

import argparse
import hashlib
import json
import os
from pathlib import Path

import numpy as np


ROOT = Path(__file__).resolve().parents[2]
PROTOCOL_PATH = ROOT / "scripts/quantvla_libero_dypac_protocol.json"
PROTOCOL = json.loads(PROTOCOL_PATH.read_text(encoding="utf-8"))
SUITES = tuple(PROTOCOL["benchmark"]["suites"])
ARRAY_KEYS = (
    "images",
    "wrist_images",
    "states",
    "prompts",
    "task_ids",
    "suite_ids",
    "task_indices",
    "env_seeds",
    "replan_indices",
    "selection_rows",
    "action_noises",
    "action_noises_b",
    "teacher_actions",
)


def sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(8 * 1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def atomic_npz(path: Path, arrays: dict[str, np.ndarray]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = Path(str(path) + f".tmp.{os.getpid()}")
    with temporary.open("wb") as handle:
        np.savez_compressed(handle, **arrays)
    temporary.replace(path)


def atomic_json(path: Path, payload: dict) -> None:
    temporary = Path(str(path) + f".tmp.{os.getpid()}")
    temporary.write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    temporary.replace(path)


def validate_sequences(arrays: dict[str, np.ndarray], *, selection_only: bool) -> None:
    expected_replans = tuple(PROTOCOL["benchmark"]["retained_replan_indices"])
    groups: dict[tuple[str, int, int], list[int]] = {}
    for index in range(len(arrays["states"])):
        if selection_only and not bool(arrays["selection_rows"][index]):
            continue
        key = (
            str(arrays["suite_ids"][index]),
            int(arrays["task_indices"][index]),
            int(arrays["env_seeds"][index]),
        )
        groups.setdefault(key, []).append(int(arrays["replan_indices"][index]))
    for key, replans in groups.items():
        if tuple(replans) != expected_replans:
            raise ValueError(f"incomplete ordered replan sequence {key}: {replans}")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--shard-dir", required=True)
    parser.add_argument("--out", required=True, help="Merged 256-row calibration NPZ")
    parser.add_argument("--selection-out", required=True, help="Merged 144-row selection NPZ")
    args = parser.parse_args()
    shard_dir = Path(args.shard_dir).expanduser().resolve()
    out = Path(args.out).expanduser().resolve()
    selection_out = Path(args.selection_out).expanduser().resolve()
    if out.exists() or selection_out.exists():
        raise FileExistsError("refusing to overwrite merged DyPAC calibration artifacts")

    banks: dict[str, list[np.ndarray]] = {key: [] for key in ARRAY_KEYS}
    shards: list[dict] = []
    expected_protocol_sha = sha256_file(PROTOCOL_PATH)
    source_metadata_hashes: set[str] = set()
    for suite in SUITES:
        path = shard_dir / f"{suite}.npz"
        sidecar_path = Path(str(path) + ".json")
        sidecar = json.loads(sidecar_path.read_text(encoding="utf-8"))
        if sidecar["protocol_sha256"] != expected_protocol_sha:
            raise ValueError(f"{suite}: protocol drift")
        if sidecar["sha256"] != sha256_file(path):
            raise ValueError(f"{suite}: shard hash mismatch")
        if sidecar["rows"] != 64 or sidecar["selection_rows"] != 36:
            raise ValueError(f"{suite}: unexpected shard coverage")
        metadata_bytes = json.dumps(
            sidecar["source_server_metadata"], sort_keys=True, separators=(",", ":")
        ).encode("utf-8")
        source_metadata_hashes.add(hashlib.sha256(metadata_bytes).hexdigest())
        with np.load(path, allow_pickle=False) as archive:
            missing = set(ARRAY_KEYS) - set(archive.files)
            if missing:
                raise ValueError(f"{suite}: missing arrays {sorted(missing)}")
            for key in ARRAY_KEYS:
                value = np.asarray(archive[key])
                if len(value) != 64:
                    raise ValueError(f"{suite}/{key}: {len(value)} != 64")
                banks[key].append(value)
        shards.append(
            {"suite": suite, "path": str(path), "sha256": sidecar["sha256"]}
        )

    arrays = {key: np.concatenate(values, axis=0) for key, values in banks.items()}
    if len(arrays["states"]) != 256 or int(arrays["selection_rows"].sum()) != 144:
        raise RuntimeError("merged calibration coverage drift")
    if set(map(str, arrays["suite_ids"])) != set(SUITES):
        raise RuntimeError("merged calibration misses a LIBERO suite")
    if len(set(map(str, arrays["task_ids"]))) != 40:
        raise RuntimeError("merged calibration misses LIBERO tasks")
    if set(map(int, arrays["env_seeds"])) & set(
        PROTOCOL["benchmark"]["formal_initial_state_indices"]
    ):
        raise RuntimeError("calibration overlaps formal initial states")
    validate_sequences(arrays, selection_only=False)
    validate_sequences(arrays, selection_only=True)

    atomic_npz(out, arrays)
    mask = arrays["selection_rows"].astype(bool)
    selection = {key: value[mask] for key, value in arrays.items()}
    atomic_npz(selection_out, selection)
    payload = {
        "schema_version": 1,
        "kind": "dypac_libero_fp16_onpolicy_merged",
        "protocol_id": PROTOCOL["protocol_id"],
        "protocol_sha256": expected_protocol_sha,
        "rows": 256,
        "selection_rows": 144,
        "tasks": 40,
        "suites": list(SUITES),
        "initial_state_indices": [0, 1, 2],
        "formal_initial_state_indices": PROTOCOL["benchmark"]["formal_initial_state_indices"],
        "overlap_with_formal": False,
        "uses_success_labels": False,
        "uses_test_rollout_feedback": False,
        "source_server_metadata_sha256s": sorted(source_metadata_hashes),
        "shards": shards,
        "npz": str(out),
        "sha256": sha256_file(out),
        "selection_npz": str(selection_out),
        "selection_sha256": sha256_file(selection_out),
    }
    atomic_json(Path(str(out) + ".json"), payload)
    atomic_json(
        Path(str(selection_out) + ".json"),
        {**payload, "kind": "dypac_libero_selection_buffer", "rows": 144, "npz": str(selection_out), "sha256": payload["selection_sha256"]},
    )
    print(json.dumps(payload, indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
