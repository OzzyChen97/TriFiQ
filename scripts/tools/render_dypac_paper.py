...
def main_table(data: dict[str, dict[str, Any]]) -> str:
    ...
    rows = [
        ("FP16 teacher", "--", fp, "1.993", "1.00$\\times$"),
        ("\\quantvla W4A8", "116 / 0", q, "0.898", "2.22$\\times$"),
        ("GDSQ-VLA (previous)", "100 / 16", g, "--", "--"),
        ("\\textbf{\\method (Ours)}", "100 / 16", c, "\\best{0.896}", "\\best{2.22$\\times$}"),
    ]
    rendered = []
    for name, mask, result, size, comp in rows:
        split = result["split_task_macro_success_rate"]
        all_sr = pct(result["task_macro_success_rate"])
        if "Ours" in name:
            all_sr = f"\\best{{{all_sr}}}"
        rendered.append(
            f"{name} & {mask} & {pct(split['atomic_seen'])} & {pct(split['composite_seen'])} & "
            f"{pct(split['composite_unseen'])} & {all_sr} & {size} & {comp} \\\\"
        )
    body = "\n".join(rendered)
...
def pi05_table(data: dict[str, dict[str, Any]]) -> str:
    plan = data["pi05_plan"]
    main = data["pi05_main_plan"]
    quick = data["pi05_quick"]
    main_gib = main["table1_total_static_bytes"] / 2**30
    plan_gib = plan["table1_total_static_bytes"] / 2**30
    return f"""% AUTO-GENERATED by scripts/tools/render_dypac_paper.py; DO NOT EDIT.
\\begin{{table}}[t]
\\centering
...
\\midrule
Previous main plan & 80 & 100 & {quick['main_successes']}/100 & {main_gib:.3f} & {main['table1_total_static_compression']:.3f}$\\times$ \\\\
\\method anchor & 121 & 59 & {quick['candidate_successes']}/100 & \\best{{{plan_gib:.3f}}} & \\best{{{plan['table1_total_static_compression']:.3f}$\\times$}} \\\\
\\bottomrule
\\end{{tabular}}
\\end{{table}}
"""
...
def generated_files(data: dict[str, dict[str, Any]], registry: dict[str, Any]) -> dict[Path, str]:
    ...
    return {
        ...
        PAPER / "tables/pi05_anchor.tex": pi05_table(data),
